...
const filterLongBooks = function (books) {
  return books.filter((book) => book.pages >= 250);
};
...
const isOrderPlacedInLast30Days = function (
  currentDay,
  currentMonth,
  currentYear
) {
  return function (order) {
    orderDay = order.orderDate.slice(0, 4);
    orderMonth = order.orderDate.slice(5, 7);
    orderYear = order.orderDate.slice(8, 9);
  };
};

const filterRecentOrders = function (orders) {
  const currentDate = "2024 - 12-01";
  const currentDay = currentDate.slice(0, 4);
  const currentMonth = currentDate.slice(5, 7);
  const currentYear = currentDate.slice(8, 9);

  return orders.filter(
    isOrderPlacedInLast30Days(currentDay, currentMonth, currentYear)
  );
};
...
const filterBelowAveragePrice = function (products) {
  ...
  const average =
    productsPrice.reduce((sum, number) => sum + number, 0) / products.length;
...
const isStudentPassed = function (student) {
  return student.subjects[0].passed && student.subjects[1].passed;
};
...
const isBirthDayInThisMonth = function (currentMonth) {
  return (person) => person.birthDate.slice(5, 7) === currentMonth;
};
...
const filterBirthdaysThisMonth = function (people) {
  ...
  return people.filter(isBirthDayInThisMonth(currentMonth));
};
...
const filterHighValueOrders = function (orders) {
  const orderAmount = orders.map((product) => product.amount);
  const average =
    orderAmount.reduce((sum, number) => sum + number, 0) / orders.length;

  return orders.filter((order) => average < order.amount);
};
...
const filterTopRatedBooks = function (books) {
  ...
  const average =
    ratings.reduce((sum, number) => sum + number, 0) / books.length;

  return books.filter((product) => product.rating > average);
};
...
const filterHighSalaryEmployees = function (employees) {
  ...
  const average =
    employeeSalary.reduce((sum, number) => sum + number, 0) / employees.length;
...
const findMedian = function (sortedPopulation) {
  const midPoint = Math.floor(sortedPopulation.length / 2);

  if (sortedPopulation.length % 2 === 1) {
    return sortedPopulation[midPoint];
  }

  return (sortedPopulation[midPoint] + sortedPopulation[midPoint - 1]) / 2;
};

const filterCitiesAboveMedianPopulation = function (cities) {
  const sortedPopulation = cities
    .map((city) => city.population)
    .toSorted((a, b) => a - b);
  const median = findMedian(sortedPopulation);

  return cities.filter((city) => city.population > median);
};

console.log = filterCitiesAboveMedianPopulation([
  { name: "City A", population: 1000 },
  { name: "City B", population: 1000 },
...
const filterPopularPosts = function (posts) {
  ...
  const average =
    likes.reduce((sum, average) => sum + average, 0) / posts.length;

  return posts.filter((post) => post.likes > average);
};

console.log = filterPopularPosts([
  { postId: 1, likes: 100 },
  { postId: 2, likes: 200 },
  { postId: 3, likes: 150 },
]);
...
const filterActiveUsersByPostCount = function (users) {
  ...
  const average =
    postCount.reduce((sum, number) => sum + number, 0) / users.length;

  return users.filter((user) => user.postCount > average);
};

console.log = filterActiveUsersByPostCount([
...
console.log = filterBySubjectGrade(
  [
...
// filter photos with a minimum number of likes [{id: 1, likes: 100}, {id: 2, likes: 50}] => [{id: 1, likes: 50}]

const filterByLikes = function (photos, likes) {
  return photos.filter((phote) => phote.likes < likes);
};
...
const filterByPostCount = function (users, postCount) {
  return users.filter((user) => user.posts === 10);
};

console.log = filterByPostCount(
  [
